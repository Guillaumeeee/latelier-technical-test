...
describe('Testing API endpoints', () => {
  describe('GET /players', () => {
    ...
    test('should respond with header Access-Control-Allow-Origin set to *', async () => {
      const res = await request(app).get(`${basePathUrl}/players`);
  ...
  describe('GET /player/:id', () => {
    ...
    test('should respond with status 404 and not found from getPlayers', async () => {
      const res = await request(app).get(`${basePathUrl}/player/1000000`);
      expect(res.statusCode).toBe(404);
    });
    test('should respond with status 400 and bad request because params id not set / undefined / of wrong type', async () => {
  ...
  describe('GET /statistics', () => {
    test('should respond with status 200 and all data from getStatistics', async () => {
